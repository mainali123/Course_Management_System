...
public class Student extends javax.swing.JFrame {
    ...
    public static void showReportDataInTableFromDb(){   // Method to show report data in table from database
        Statement statement = JDBC.getStatement();  // Object of Statement class to execute queries

        try {
            String query = "SELECT isGenerated FROM report WHERE Student_ID = '" + LoginPanel.id + "'";
            ResultSet resultSet = statement.executeQuery(query);
            // Checking if report is generated or not with the first row of the result set
            if (!resultSet.isBeforeFirst()) {   // If report is empty
                JOptionPane.showMessageDialog(null, "Report is not generated yet");
            } else {    // If report is generated
                resultSet.next();
                if (resultSet.getString("isGenerated").equals("0")) {   // If report is not generated
                    JOptionPane.showMessageDialog(null, "Report is not generated yet");
                } else {
                    ...
                    while (resultSet.next()) {
                        ...
                        reportDefaultTableModel.addRow(new Object[] {courseID, moduleCode, marks}); // Adding data to table
                }
            }
    ...
    public static void showInstructorDataInTableFromDb(){   // Method to show instructor data in table from database
    ...
    public static void showAssignmentDataInTableFromDb() {  // Method to show assignment data in table from database
        Statement statement = JDBC.getStatement();
    ...
    private void initComponents() {
        ...
        jTable1.addMouseListener(new MouseAdapter() {
        	...
        	public void mouseClicked(MouseEvent e) {
                // Asking user weather he have submitted assignment on canvas or not
                ...
                if (dialogResult == JOptionPane.YES_OPTION) {
                    ...
                    try {
                        ...
                        JOptionPane.showMessageDialog(null, "Assignment submitted successfully");   // If yes then update the database
                    } catch (SQLException e1) {
                        e1.printStackTrace();
                    }
                } else {    // If no then update the database
                    int row = jTable1.getSelectedRow();
                    String assignmentNumber = jTable1.getModel().getValueAt(row, 0).toString();
                    try {
                        Statement statement = JDBC.getStatement();
                        ...
                        statement.executeUpdate(query);
                        JOptionPane.showMessageDialog(null, "Assignment submitted successfully");   // If no then update the database
                    } catch (SQLException e1) {
                        e1.printStackTrace();
                    }
                }
        ...
        Statement statement = JDBC.getStatement();
        String query1 = "SELECT Course_ID FROM courses";    // query to get all the course ids
        ...
        courseIds.add("Select Course"); // add a default value to the list
        try {
        while (resultSet1.next()) {
        try {
        courseIds.add(resultSet1.getString("Course_ID"));   // add all the course ids to the list
        } catch (SQLException e) {
        e.printStackTrace();
        }
        }
        ...
        String[] courseIdArray = courseIds.toArray(new String[0]);  // convert the list to an array
        
        studentCourseIdComboBox.setModel(new javax.swing.DefaultComboBoxModel<>(courseIdArray));    // set the array as the model for the combobox, so that the combobox will have all the real time course ids
    ...
    private void addStudentButtonActionPerformed(java.awt.event.ActionEvent evt) {    
        ...
        if (studentNameTextField.getText().isEmpty() || studentAddressTextField.getText().isEmpty() || studentDobTextField.getText().contentEquals("DD/MM/YYYY") || studentEmailTextField.getText().isEmpty() || studentPasswordTextField.getText().isEmpty() || (String) studentCourseIdComboBox.getSelectedItem() == "Select Course" || studentCitizenshipTextField.getText().isEmpty() || studentCollegeEmailTextField.getText().isEmpty() || studentStudentIdTextField.getText().isEmpty()) {
        ...
        } else {    // If all the fields are filled then adding the student to the database
            ...
            try {
                ...
                if (checkResult.next()) {
                    // If student Id already exists in the database then displaying error message
